...
from theme import draw_rounded_rect
...
class BlockManager:
    ...
    def create_block(self, kind: str = "text"):
        ...
        color = theme.CODE_BG if kind == "code" else theme.BLOCK_PALETTE[self._next_id % len(theme.BLOCK_PALETTE)]
        block = {
            "id": self._next_id,
            "kind": kind,
    ...
    def draw(self, frame, hovering_text_button: bool, hovering_code_button: bool,
        ...
        for block in self.blocks:
            ...
            # subtle drop shadow -- a small, cheap depth cue that makes
            # flat rectangles read as "raised" panels instead of a flat paint job
            shadow_tl = (top_left[0] + SHADOW_OFFSET, top_left[1] + SHADOW_OFFSET)
            shadow_br = (bottom_right[0] + SHADOW_OFFSET, bottom_right[1] + SHADOW_OFFSET)
            draw_rounded_rect(frame, shadow_tl, shadow_br, BLOCK_RADIUS, SHADOW_COLOR, -1)

            draw_rounded_rect(frame, top_left, bottom_right, BLOCK_RADIUS, block["color"], -1)
            if block["id"] == linking_from_id:
                border, thickness = theme.ACCENT, 3
            elif block["id"] == selected_id:
                border, thickness = theme.SUCCESS, 3
            else:
                border, thickness = theme.BORDER, 2
        ...
        for block in self.blocks:
            ...
            if target is not None:
                cv2.arrowedLine(frame, self.center_of(block), self.center_of(target),
                                 theme.LINK, 2, tipLength=0.12, line_type=cv2.LINE_AA)

        # "+" (text) / "<>" (code) buttons, top-left, on a shared glass backing
        bx1, by1, _, _ = BUTTON_TEXT_RECT
        _, _, bx2, by2 = BUTTON_CODE_RECT
        theme.draw_glass_panel(frame, (bx1 - 8, by1 - 8), (bx2 + 8, by2 + 8), radius=14)

        color = theme.ACCENT if hovering_text_button else theme.TEXT_MUTED
        x1, y1, x2, y2 = BUTTON_TEXT_RECT
        draw_rounded_rect(frame, (x1, y1), (x2, y2), 10, color, 2)
        cx, cy = (x1 + x2) // 2, (y1 + y2) // 2
        cv2.line(frame, (cx - 10, cy), (cx + 10, cy), color, 2, cv2.LINE_AA)
        cv2.line(frame, (cx, cy - 10), (cx, cy + 10), color, 2, cv2.LINE_AA)

        ccolor = theme.ACCENT if hovering_code_button else theme.TEXT_MUTED
        x1, y1, x2, y2 = BUTTON_CODE_RECT
        draw_rounded_rect(frame, (x1, y1), (x2, y2), 10, ccolor, 2)
        theme.put_text(frame, "<>", (x1 + 3, y2 - 13), scale=0.5, color=ccolor, thickness=2)

        # trash button, bottom-right -- fills red as a held drag-to-delete progresses
        tx1, ty1, tx2, ty2 = self.trash_rect
        theme.draw_glass_panel(frame, (tx1 - 8, ty1 - 8), (tx2 + 8, ty2 + 16), radius=14)
        trash_color = theme.DANGER if trash_progress > 0 else theme.TEXT_MUTED
        draw_rounded_rect(frame, (tx1, ty1), (tx2, ty2), 8, trash_color, 2)
        lx1, ly1, lx2, ly2 = tx1 + 8, ty1 + 12, tx2 - 8, ty1 + 16
        cv2.rectangle(frame, (lx1, ly1), (lx2, ly2), trash_color, -1, cv2.LINE_AA)
        for i in range(3):
            lx = tx1 + 14 + i * 8
            cv2.line(frame, (lx, ty1 + 18), (lx, ty2 - 8), trash_color, 2, cv2.LINE_AA)
        if trash_progress > 0:
            bar_w = int((tx2 - tx1) * trash_progress)
            cv2.rectangle(frame, (tx1, ty2 + 4), (tx1 + bar_w, ty2 + 8), theme.DANGER, -1, cv2.LINE_AA)
    ...
    def draw_fullscreen(self, frame, block, hovering_arrow: bool, run_progress: float = 0.0):
        ...
        if block["kind"] == "code":
            draw_rounded_rect(frame, (x1, y1), (x2, y2), 30, theme.CODE_BG, -1)
            draw_rounded_rect(frame, (x1, y1), (x2, y2), 30, theme.SUCCESS, 3)

            split_y = y1 + int((y2 - y1) * 0.6)
            code_scale, code_thickness, line_height = 0.75, 1, 30

            theme.put_text(frame, "CODE -- poing ferme pour executer", (x1 + 30, y1 + 40),
                            scale=0.6, color=theme.TEXT_MUTED)
            ...
            for line in lines[: max(1, (split_y - y1 - 90) // line_height)]:
                theme.put_text(frame, line, (x1 + 30, ty), scale=code_scale, color=theme.SUCCESS, thickness=code_thickness)
                ty += line_height

            cv2.line(frame, (x1 + 20, split_y), (x2 - 20, split_y), theme.BORDER, 1, cv2.LINE_AA)
            theme.put_text(frame, "SORTIE", (x1 + 30, split_y + 30), scale=0.6, color=theme.TEXT_MUTED)
            ...
            for line in out_lines[: max(1, (y2 - split_y - 90) // line_height)]:
                theme.put_text(frame, line, (x1 + 30, ty), scale=code_scale, color=theme.TEXT_PRIMARY, thickness=code_thickness)
                ty += line_height

            if run_progress > 0:
                bar_w = int((x2 - x1 - 60) * run_progress)
                cv2.rectangle(frame, (x1 + 30, y2 - 20), (x1 + 30 + bar_w, y2 - 12), theme.SUCCESS, -1, cv2.LINE_AA)
        else:
            draw_rounded_rect(frame, (x1, y1), (x2, y2), 40, block["color"], -1)
            draw_rounded_rect(frame, (x1, y1), (x2, y2), 40, theme.TEXT_PRIMARY, 3)
            ...
            for line in lines[:max_lines]:
                theme.put_text(frame, line, (x1 + 40, ty), scale=font_scale, color=theme.TEXT_PRIMARY, thickness=thickness)
                ty += line_height
        ...
        ax2 = ax1 + ARROW_W
        ay2 = ay1 + ARROW_H
        color = theme.ACCENT if hovering_arrow else theme.TEXT_PRIMARY
        cv2.arrowedLine(frame, (ax1, (ay1 + ay2) // 2), (ax2, (ay1 + ay2) // 2), color, 6,
